...
if (!API_KEY) {
    ...
    const beginButton = document.getElementById('begin-journey-button') as HTMLButtonElement | null;
...
const beginJourneyButton = document.getElementById('begin-journey-button') as HTMLButtonElement;
const choiceButtons = document.querySelectorAll('.choice-button');
const questionsContainer = document.getElementById('questions-container');
const questionsTitle = document.getElementById('questions-title');
const nextToRoomDescriptionButton = document.getElementById('next-to-room-description-button') as HTMLButtonElement;
const roomDescriptionInput = document.getElementById('room-description-input') as HTMLTextAreaElement;
const getThemeIdeasButton = document.getElementById('get-theme-ideas-button') as HTMLButtonElement;
...
const getRoomChangesButton = document.getElementById('get-room-changes-button') as HTMLButtonElement;
const roomImageUpload = document.getElementById('room-image-upload') as HTMLInputElement;
...
function showQuizStep(stepId: string) {
  ...
  const progress = document.getElementById('quiz-progress') as HTMLElement | null;
  const stepNumEl = document.getElementById('quiz-step-num') as HTMLElement | null;
  if (progress) progress.style.width = `${Math.min(stepNum, totalSteps) / totalSteps * 100}%`;
  if (stepNumEl) stepNumEl.textContent = `${Math.min(stepNum, totalSteps)}`;
}

function updateUIForStep(step: string) {
    currentStep = step;
    [welcomeStep, transformationChoiceStep, questionsStep, roomDescriptionStep, resultsStep].forEach(s => s?.classList.add('hidden'));
    if (statusArea) statusArea.textContent = ''; // Clear status on step change

    switch (step) {
        case 'welcome':
            welcomeStep?.classList.remove('hidden');
            if(headerSubtitle) headerSubtitle.textContent = "Your personal guide to a beautifully themed space.";
            break;
        case 'transformationChoice':
            transformationChoiceStep?.classList.remove('hidden');
            if(headerSubtitle) headerSubtitle.textContent = "Let's start with your core inspiration.";
            break;
        case 'questions':
        ...
        case 'roomDescription':
            roomDescriptionStep?.classList.remove('hidden');
             if(headerSubtitle) headerSubtitle.textContent = "Tell us about the canvas for your transformation.";
            roomDescriptionInput?.focus();
            break;
        case 'results':
            resultsStep?.classList.remove('hidden');
            if(headerSubtitle) headerSubtitle.textContent = "Here are your personalized suggestions!";
            break;
    }
}

function createQuestionElement(id: string, labelText: string, type: 'text' | 'textarea' | 'select', options?: {value: string, text: string}[]) {
...
function renderQuestions() {
    if (!questionsContainer || !questionsTitle || !userData.transformationType) return;
    questionsContainer.innerHTML = ''; // Clear previous questions
    userData.questions = {}; // Reset stored answers for this section

    let questionsConfig: {id: string, label: string, type: 'text' | 'textarea' | 'select', options?: {value: string, text: string}[]}[] = [];
    let titleText = '';
...
function hideGetRoomChangesButton() {
    if (getRoomChangesButton) getRoomChangesButton.style.display = 'none';
    if (roomImageUpload) roomImageUpload.style.display = 'none';
    if (aiImageResult) aiImageResult.innerHTML = '';
}
...
async function handleFinalSubmit() {
    ...
    if (!API_KEY) {
        if (statusArea) statusArea.textContent = 'API Key is not configured. Cannot process request.';
        return;
    }

    userData.roomDescription = roomDescriptionInput.value.trim();
    if (!userData.roomDescription) {
        if (statusArea) statusArea.textContent = 'Please describe your room.';
        roomDescriptionInput.focus();
        return;
    }
    ...
    function shuffleQuotes(arr: string[]) {
    ...
    try {
        ...
        if (statusArea) {
            ...
            if (typeof quoteInterval !== 'undefined') clearInterval(quoteInterval);
        }
...
choiceButtons.forEach(button => {
    button.addEventListener('click', () => {
        userData.transformationType = (button as HTMLElement).dataset.choice;
...
nextToRoomDescriptionButton?.addEventListener('click', () => {
    ...
    if (userData.transformationType !== 'custom' && userData.questions && Object.values(userData.questions).every(val => val.trim() === '')) {
         if (statusArea) statusArea.textContent = 'Please answer at least one question to help personalize your theme.';
         return;
    }
     if (userData.transformationType === 'custom' && (!userData.questions || !userData.questions['custom-ideas'] || userData.questions['custom-ideas'].trim() === '')) {
         if (statusArea) statusArea.textContent = 'Please describe your custom vision.';
         return;
    }
...
const testPrototypeBtn = document.getElementById('test-prototype-btn') as HTMLButtonElement | null;
const comingSoonMain = document.querySelector('.coming-soon-main') as HTMLElement | null;
const quizSteps = document.getElementById('quiz-steps') as HTMLElement | null;

if (quizSteps) quizSteps.style.display = 'none';
if (comingSoonMain) comingSoonMain.style.display = '';

testPrototypeBtn?.addEventListener('click', () => {
    if (comingSoonMain) comingSoonMain.style.display = 'none';
    if (quizSteps) quizSteps.style.display = '';
    showQuizStep('transformation-choice-step');
});

const letsBeginBtn = document.getElementById('lets-begin-btn') as HTMLButtonElement | null;
if (letsBeginBtn) {
  letsBeginBtn.addEventListener('click', () => {
    if (letsBeginBtn.parentElement) letsBeginBtn.parentElement.style.display = 'none';
    if (quizSteps) quizSteps.style.display = '';
    showQuizStep('transformation-choice-step');
  });
}
...
if (API_KEY) {
    updateUIForStep('welcome');
     ...
     if (welcomeStep) welcomeStep.classList.remove('hidden'); // Show welcome but button will be disabled
     if (beginJourneyButton) beginJourneyButton.disabled = true;
     if (statusArea) statusArea.innerHTML = '<strong>API Key Missing:</strong> Application features are limited. Please contact administrator.';
